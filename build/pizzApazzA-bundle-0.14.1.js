...
class Z4DrawingToolPreview extends JSDropDown {
  ...
  constructor() {
    ...
    this.name.getStyle().width = (Z4DrawingToolPreview.PREVIEW_SIZE + 35) + "px";
    ...
    this.eye.setText(Z4DrawingToolPreview.VISIBLE_LAYER_CONTENT);
    ...
    selector.setText(Z4DrawingToolPreview.SELECTED_LAYER_CONTENT);
    ...
    selector.addActionListener(event => {
      document.querySelectorAll(".z4layerpreview .z4layerpreview-selector").forEach(element => element.textContent = Z4DrawingToolPreview.UNSELECTED_LAYER_CONTENT);
      selector.setText(Z4DrawingToolPreview.SELECTED_LAYER_CONTENT);
   ...
   */
   setLayer(canvas, layer) {
    ...
    let h = 0.0;
    if (ratio > 1) {
      w = Z4DrawingToolPreview.PREVIEW_SIZE;
      h = Z4DrawingToolPreview.PREVIEW_SIZE / ratio;
    } else {
      w = Z4DrawingToolPreview.PREVIEW_SIZE * ratio;
      h = Z4DrawingToolPreview.PREVIEW_SIZE;
    }
    this.zoom = Math.min(w / d.width, h / d.height);
    this.preview.setAttribute("width", "" + w);
    this.preview.setAttribute("height", "" + h);
    this.preview.getStyle().marginTop = (Z4DrawingToolPreview.PREVIEW_SIZE - h - 1) / 2 + "px";
    this.preview.getStyle().marginBottom = (Z4DrawingToolPreview.PREVIEW_SIZE - h - 1) / 2 + "px";
    this.preview.getStyle().marginLeft = (Z4DrawingToolPreview.PREVIEW_SIZE - w - 1) / 2 + "px";
    this.preview.getStyle().marginRight = (Z4DrawingToolPreview.PREVIEW_SIZE - w - 1) / 2 + "px";
