...
public class Z4GeometricShapePreview extends JSDropDown {
   ...
   */
  public void setSelectedControlPoint(int selectedControlPoint) {
    this.selectedControlPoint = selectedControlPoint;
    this.radios.$get(selectedControlPoint).setSelected(true);
   ...
   */
  public void setSelectedControlPointPosition(int x, int y) {
    this.xSlider.setValue(x);
    this.xSpinner.setValue(x);
    this.ySlider.setValue(y);
    this.ySpinner.setValue(y);

    Z4GeometricShape newShape = this.shape.fromDataChanged(this.shape.getControlPoints(), x, y, this.selectedControlPoint, 0, -1, this.canvas.getSize().width, this.canvas.getSize().height);
    this.canvas.replaceGeometricShape(this.shape, newShape, this.selectedControlPoint);
    this.setGeometriShape(this.canvas, newShape);
  }
  ...
  public void drawShape() {
    if ($exists(this.shape)) {
      $Path2D path2D = this.shape.getPath2D(false);
